...
app.get("/api/weather", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const response = yield (0, node_fetch_1.default)(`https://api.openweathermap.org/data/2.5/forecast?units=metric&lat=${lat}&lon=${lon}&appid=${process.env.GET_WEATHER_FORECAST_API_KEY}`);
    if (!response.ok)
        return res.status(500).json({
            error: response.text ||
                response.statusText ||
